...
- **Endpoints**: Micropub via the shim at `micropub.stormfield.house/micropub` (LAN/tailnet-only; babylon repo, `micropub` stack — static bearer token, no IndieAuth). WebMention endpoint at `webmention.andystevens.name` (go-jamming on the Linode VPS — the one endpoint that must stay publicly reachable). No authorization/token endpoints: web sign-in works via RelMeAuth (rel="me" GitHub link, profile links back).
- **ActivityPub**: self-hosted. `.well-known/webfinger` + static actor served via the Bunny Edge Script (`edge-script/main.mjs`); dynamic endpoints at `ap.andystevens.name`; outbound delivery runner-side (`scripts/deliver-to-activitypub.mjs`).
- **Webmention sending**: Triggered by the build workflow (`scripts/send-webmentions.mjs`), which calls go-jamming via `PUT https://webmention.andystevens.name/webmention/andystevens.name/{GO_JAMMING_TOKEN}`. Go-jamming discovers the feed via `/feed` (redirects to `/feed.xml`). The RSS feed must include `content:encoded` (via the `content` field in `@astrojs/rss` items) for go-jamming to find outbound links.
- **Bluesky syndication**: Triggered by the build workflow (`scripts/deliver-to-bluesky.mjs`), ledger-driven (see `src/lib/syndicate.mjs`), gated per-post by the `syndication` frontmatter array via `wantsSyndication()`. Requires `BLUESKY_HANDLE` and `BLUESKY_APP_PASSWORD` Forgejo Action secrets.
- **Posting**: Sveltia CMS at `/admin/` (static page, browser -> Forgejo API, config in `public/admin/config.yml` — keep its collections in sync with `src/content.config.ts`) and the `/post/` bookmarklet page -> Micropub shim. IndieKit + MongoDB were decommissioned from the Linode VPS (2026-07-11); the VPS now runs only go-jamming.
